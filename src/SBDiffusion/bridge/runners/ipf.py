...
############################
# MODIFIED Regularized Implementation based on IPF of forward and backward diffusion process.
############################
...
class IPFBase(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.accelerator = Accelerator(cpu=args.device == 'cpu')
...
class IPFSequential(IPFBase):
    ...
    def ipf_step(self, forward_or_backward, n):
        ...
        for i in tqdm(range(self.num_iter+1)):
            ...
            loss = F.mse_loss(pred, out) + self.lam * pred.abs().mean()
